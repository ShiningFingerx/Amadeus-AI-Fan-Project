...
import type { UserProfile, AmadeusState } from '../types';
...
const AmadeusAvatar: React.FC<{ 
    ...
    isLoading, userProfile, amadeusState, onSignOut, onOpenAbout, onOpenSettings, onOpenMemories, onOpenKurisuProfile, isMusicPlaying, isMusicLoaded, onToggleMusic, onUploadMusic,
    onViewAvatar, isGlitching
 }) => {
  return (
    <div className="glass-panel p-6 rounded-lg h-full flex flex-col justify-between overflow-y-auto scrollbar-thin-amber">
        ...
        <div className="relative flex justify-center items-center my-8 h-48">
          ...
          <svg width="200" height="200" viewBox="0 0 200 200" className="absolute">
            ...
            <g style={{ animation: `pulse-glow 3s ease-in-out infinite`, filter: 'url(#glow)' }}>
                ...
                {[...Array(8)].map((_, i) => (
                    <line key={i} x1="100" y1="100" x2="100" y2="40" stroke="#ff8c00" strokeWidth="1" opacity="0.6" style={{transform: `rotate(${i * 45}deg)`, transformOrigin: 'center'}}/>
                ))}
            </g>
          </svg>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="pt-4 mt-4 border-t border-amber-500/30 grid grid-cols-2 gap-4">
         ...
         <div className="pt-4 mt-2 border-t border-amber-500/30 grid grid-cols-1 gap-4">
             <button
